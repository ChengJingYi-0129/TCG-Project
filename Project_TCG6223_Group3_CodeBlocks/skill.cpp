...
{
    // Draw the selected character skill effect.
    ...
    if (!useCharacter1)
    {
       if (skillIndex != 2) // Hirono: vine spike skill
        {
            float startX = -3.6f;
            float endX = 3.6f;
            float startZ = -1.5f;
        ...
        else // Hirono: vine tornado skill
        {
            glPushMatrix();
    ...
    {
        if (skillIndex != 2) // Twinkle-Twinkle: apple star skill
        {
            glPushMatrix();
        ...
        else // Twinkle-Twinkle: apple tree skill
        {
            if (treeScale < 1.0f) {
                treeScale += 0.8f;
